...
locals {
  ...
  container_image     = "${local.container_image_base}/cap-frontend:rooftopsolar"
}
...
module "alb" {
  ...
  # Add a lifecycle policy to force target group replacement
  target_group_name_suffix = formatdate("YYYYMMDD-HHmmss", timestamp())  # This will force a new target group
}
  ...
  container_secrets = [
    {
      ...
      valueFrom = "${aws_secretsmanager_secret.rooftopsolar_env.arn}:LANGCHAIN_API_KEY::"
    },
    {
      name      = "LANGGRAPH_API_URL"
      valueFrom = "${aws_secretsmanager_secret.rooftopsolar_env.arn}:LANGGRAPH_API_URL::"
    },
    {
      name      = "NEXT_PUBLIC_LANGGRAPH_ASSISTANT_ID"
      valueFrom = "${aws_secretsmanager_secret.rooftopsolar_env.arn}:NEXT_PUBLIC_LANGGRAPH_ASSISTANT_ID::"
    }
  ]
